    ...
    if ($opts{has_body_access}) {
        $builder->comment('HTTP/1.1: Find body after CRLF CRLF')
          ...
          ->if('body_start')
            ...
            ->blank
            ->comment('If a proxy/CDN split headers and body across TCP segments,')
            ->comment('the first recv() got only the headers. Drain Content-Length bytes.')
            ->line('long _content_length = -1;')
            ->line('{ const char* _h = recv_buf;')
            ->line('  const char* _hend = body - 4;')
            ->line('  while (_h < _hend) {')
            ->line('    const char* _nl = (const char*)memchr(_h, \'\\n\', _hend - _h);')
            ->line('    const char* _lend = _nl ? _nl : _hend;')
            ->line('    if (_lend - _h > 15 && (_h[0] == \'C\' || _h[0] == \'c\') &&')
            ->line('        strncasecmp(_h, "Content-Length:", 15) == 0) {')
            ->line('      const char* _v = _h + 15;')
            ->line('      while (_v < _lend && (*_v == \' \' || *_v == \'\\t\')) _v++;')
            ->line('      _content_length = strtol(_v, NULL, 10);')
            ->line('      break;')
            ->line('    }')
            ->line('    if (!_nl) break;')
            ->line('    _h = _nl + 1;')
            ->line('  } }')
            ->line('while (_content_length > 0 && body_len < _content_length && len < (ssize_t)(RECV_BUF_SIZE - 1)) {')
            ->line('  long _want = _content_length - body_len;')
            ->line('  if (_want > (long)(RECV_BUF_SIZE - 1 - len)) _want = (long)(RECV_BUF_SIZE - 1 - len);')
            ->line('  struct pollfd _pfd; _pfd.fd = fd; _pfd.events = POLLIN; _pfd.revents = 0;')
            ->line('  int _pr = poll(&_pfd, 1, RECV_TIMEOUT * 1000);')
            ->line('  if (_pr <= 0) break;')
            ->line('  ssize_t _more;')
            ->line('#ifdef HYPERSONIC_TLS')
            ->line('  _more = tls_conn ? tls_recv(tls_conn, recv_buf + len, _want) : -1;')
            ->line('#else')
            ->line('  _more = recv(fd, recv_buf + len, _want, 0);')
            ->line('#endif')
            ->line('  if (_more <= 0) break;')
            ->line('  len += _more;')
            ->line('  body_len += _more;')
            ->line('}')
            ->line('recv_buf[len] = \'\\0\';')
